 ...
 * TODO Pass off route handling to a Web Worker
 */
  ...
  for await (const requestEvent of httpConn) {
    ...
    for (const route of routes) {
      if (route.action === method && route.path.test(path)) {
        ...
        // Use the routes handler to generate a Response object
        const match = route.path.exec(requestEvent.request.url);
        const params = match?.pathname.groups;
        const response = route.handler(requestEvent.request, route.path, params);
...
type RouteHandler = (req : Request, path : URLPattern, params? : Record<string, unknown>) => Response;
